...
function changeLanguage(lang) {
    ...
    const mainSubtitle = document.querySelector('.Main_title p');
    const newsColumnTitle = document.querySelector('.new_activity .col-md-8 h4');
    const announcementTitle = document.querySelector('.new_activity .col-md-4 h4');
    
    // Empty State - News
    const emptyBoxLarge = document.querySelector('.empty-box:not(.small)');
    const emptyTitle = emptyBoxLarge ? emptyBoxLarge.querySelector('h5') : null;
    const emptyText = emptyBoxLarge ? emptyBoxLarge.querySelector('p') : null;
    
    // Empty State - Announcement
    const emptyBoxSmall = document.querySelector('.empty-box.small');
    const emptyAnnouncementText = emptyBoxSmall ? emptyBoxSmall.querySelector('p') : null;
    
    // News Card Elements (if exists in future)
    const newsDate = document.querySelector('.news-date');
    const newsReadMore = document.querySelector('.news-card .btn');
    ...
    if (lang === 'th') {
        ...
        // News Page
        if (mainTitle) mainTitle.textContent = 'ข่าวสาร และประชาสัมพันธ์';
        if (mainSubtitle) mainSubtitle.textContent = 'พื้นที่สำหรับติดตามกิจกรรม และการทำงานของเรา';
        if (newsColumnTitle) newsColumnTitle.textContent = '📌 ข่าว & กิจกรรม';
        if (announcementTitle) announcementTitle.textContent = '📣 ประกาศ';
        
        // Empty States
        if (emptyTitle) emptyTitle.textContent = 'กำลังเตรียมเนื้อหา';
        if (emptyText) emptyText.innerHTML = 'ขณะนี้ยังไม่มีข่าวสารใหม่<br>โปรดติดตามการอัปเดตเร็วๆ นี้';
        if (emptyAnnouncementText) emptyAnnouncementText.textContent = 'ยังไม่มีประกาศในขณะนี้';
        
        // News Card (if exists)
        if (newsReadMore) newsReadMore.textContent = 'อ่านเพิ่มเติม';
        ...
        // News Page
        if (mainTitle) mainTitle.textContent = 'News & Announcements';
        if (mainSubtitle) mainSubtitle.textContent = 'Stay updated with our activities and operations';
        if (newsColumnTitle) newsColumnTitle.textContent = '📌 News & Activities';
        if (announcementTitle) announcementTitle.textContent = '📣 Announcements';
        
        // Empty States
        if (emptyTitle) emptyTitle.textContent = 'Content Coming Soon';
        if (emptyText) emptyText.innerHTML = 'No news available at the moment<br>Please check back soon for updates';
        if (emptyAnnouncementText) emptyAnnouncementText.textContent = 'No announcements at this time';
        
        // News Card (if exists)
        if (newsReadMore) newsReadMore.textContent = 'Read More';
